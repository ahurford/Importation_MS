...
cdf1$bestDom[cdf1$bestDom < 0 ] <- 0

df1.Int <- select(cdf1, c(date, bestInt, Est.Int))
df1.Int <- melt(df1.Int, id=c("date"))

df1.Dom <- select(cdf1, c(date, bestDom, Est.Dom))
df1.Dom <- melt(df1.Dom, id=c("date"))

#make monthly for best model
XXXX  <- select(cdf1, c(date, bestInt,bestDom  )) 
...
df2.Int <- select(cdf2, c(date, bInt, INT))
